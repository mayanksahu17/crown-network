pipeline {
    ...
    environment {
        CONTAINER_NAME = "crown_admin_dev_${BUILD_NUMBER}"
        DOCKER_IMAGE_TAG = "crown/crown_admin:dev-${BUILD_NUMBER}"
    }
    ...
    stages {
        ...
        stage('Remove Old Container') {
            steps {
                script {
                    ...
                    sh '''
                    last_container_id=$(docker ps -qf "name=crown_admin_dev*")
                    if [ -n "$last_container_id" ]; then
                        ...
                        echo "No container with name crown_admin_dev* found to kill."
                    fi
                    '''
                }
            }
        ...
        stage('Run New Container') {
            steps {
                script {
                    sh 'docker run -d --name ${CONTAINER_NAME} -p 9002:80 ${DOCKER_IMAGE_TAG}'
                }
            }
        }
    }
